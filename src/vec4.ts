...
export namespace vec4 {
	 ...
	 * Adds given vec4's
	 */
	export function add(...vs: (vec4 | number)[]): vec4 {
		let x = 0,
			y = 0,
			z = 0,
			w = 0
	 ...
	 * - {@link sub}
	 */
	export function subtract(...vs: (vec4 | number)[]): vec4 {
		if (vs.length === 0) {
			return zero
		}
	 ...
	 * Subtracts a from b
	 */
	export function delta(a: vec4 | number, b: vec4 | number): vec4 {
	 ...
	 * - {@link mul}
	 */
	export function multiply(...vs: (vec4 | number)[]): vec4 {
		let x = 1,
			y = 1,
			z = 1,
			w = 1
	 ...
	 * - {@link div}
	 */
	export function divide(...vs: (vec4 | number)[]): vec4 {
		if (vs.length === 0) {
			return one
		}
		...
		if (vs.length === 1) {
			const v = vs[0]
			if (typeof v === 'number') return [1 / v, 1 / v, 1 / v, 1 / v]
	 ...
	 * @returns
	 */
	export function step(edge: vec4 | number, v: vec4 | number): vec4 {
		if (typeof edge === 'number') edge = [edge, edge, edge, edge]
		if (typeof v === 'number') v = [v, v, v, v]
	...
	export function smoothstep(
		...
		if (typeof edge0 === 'number') edge0 = [edge0, edge0, edge0, edge0]
		if (typeof edge1 === 'number') edge1 = [edge1, edge1, edge1, edge1]
